...
leading asset axis. The TRADES backbone itself is unchanged.
...
class SharedScoreNet(nn.Module):
    ...
    def forward(self, x_t, cond_orders, t, cond_lob, asset_ids):
        """
        Parameters
        ----------
        x_t           : (B, N, K_gen, F_in)
        ...
        # Build a (1, N, K_cond, F_in) additive mask with the embedding only
        # on the first token position. (Adding to position 0 mirrors how
        # TRADES injects positional / timestep embeddings.)
        emb_mask = torch.zeros((1, N, K_cond, F_in), device=x_t.device, dtype=x_t.dtype)
        emb_mask[:, :, 0, :] = asset_vec
