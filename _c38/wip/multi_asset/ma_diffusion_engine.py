"""MultiAssetDiffusionEngine — Lightning module mirroring
:class:`models.diffusers.diffusion_engine.DiffusionEngine` but driving the
multi-asset diffuser.

Same training loop, EMA, schedule sampler, optimizer, checkpointing logic
as the single-asset engine; the only differences are that:

* the data tuple comes in with a leading ``(B, N, ...)`` axis, and
* type embedding / loss accumulation also see that asset axis.

The single-asset engine is *not* modified — both paths coexist.
"""

import os
import numpy as np
import torch
from torch import nn
import lightning
from lightning import LightningModule

import wandb
import constants as cst
from constants import LearningHyperParameter

from models.diffusers.multi_asset.ma_gaussian_diffusion import MultiAssetGaussianDiffusion
from models.diffusers.TRADES.Sampler import LossSecondMomentResampler
from utils.utils_models import pick_augmenter
from lion_pytorch import Lion
from torch_ema import ExponentialMovingAverage


class MultiAssetDiffusionEngine(LightningModule):

    def __init__(self, config, asset_universe):
        super().__init__()
        self.asset_universe = asset_universe
        self.num_assets = asset_universe.num_assets

        self.conditional_dropout = config.HYPER_PARAMETERS[LearningHyperParameter.CONDITIONAL_DROPOUT]
        self.IS_AUGMENTATION = config.IS_AUGMENTATION
        self.IS_WANDB = config.IS_WANDB
        self.augment_dim = config.HYPER_PARAMETERS[LearningHyperParameter.AUGMENT_DIM]
        self.cond_type = config.COND_TYPE
        self.cond_method = config.COND_METHOD
        self.cond_seq_size = (
            config.HYPER_PARAMETERS[LearningHyperParameter.SEQ_SIZE]
            - config.HYPER_PARAMETERS[LearningHyperParameter.MASKED_SEQ_SIZE]
        )
        self.reg_term_weight = config.HYPER_PARAMETERS[LearningHyperParameter.REG_TERM_WEIGHT]
        self.num_diffusionsteps = config.HYPER_PARAMETERS[LearningHyperParameter.NUM_DIFFUSIONSTEPS]
        self.size_type_emb = config.HYPER_PARAMETERS[LearningHyperParameter.SIZE_TYPE_EMB]
        self.size_order_emb = config.HYPER_PARAMETERS[LearningHyperParameter.SIZE_ORDER_EMB]
        self.chosen_model = config.CHOSEN_MODEL.value
        self.betas = config.BETAS
        self.training = config.IS_TRAINING
        self.test_batch_size = config.HYPER_PARAMETERS[LearningHyperParameter.TEST_BATCH_SIZE]
        self.epochs = config.HYPER_PARAMETERS[LearningHyperParameter.EPOCHS]
        self.seq_size = config.HYPER_PARAMETERS[LearningHyperParameter.SEQ_SIZE]
        self.train_losses, self.vlb_train_losses, self.simple_train_losses = [], [], []
        self.val_ema_losses = []
        self.simple_val_losses, self.vlb_val_losses = [], []
        self.min_loss_ema = np.inf
        self.min_train_loss = np.inf
        self.filename_ckpt = config.FILENAME_CKPT
        self.last_path_ckpt_ema = None
        self.optimizer = config.HYPER_PARAMETERS[LearningHyperParameter.OPTIMIZER]
        self.lr = config.HYPER_PARAMETERS[LearningHyperParameter.LEARNING_RATE]
        # Tiny LR for the P1 graph-coupling branch (gate gamma + edge/message/
        # fusion MLPs). The residual gate starts at 0; under the shared 1e-3
        # Adam LR it runs away to ~0.4 within ~1k steps (Adam gives a scalar a
        # full lr*sign step regardless of the noisy random-MLP gradient),
        # injecting unstructured noise into eps and corrupting the shared
        # trunk irreversibly. A ~1e-5 LR lets the gate open slowly while the
        # MLPs co-adapt. Env-overridable via GRAPH_LR on the launcher.
        self.graph_lr = float(getattr(config, "GRAPH_LR", 1e-5))
        self.cond_size = config.COND_SIZE

        if self.IS_AUGMENTATION:
            self.feature_augmenter = pick_augmenter(
                config.CHOSEN_AUGMENTER,
                self.size_order_emb,
                self.augment_dim,
                self.cond_size,
                self.cond_type,
                config.CHOSEN_COND_AUGMENTER,
                self.cond_method,
                # The augmenter branches on chosen_model == "TRADES"; reuse
                # that path because the score net under MA_TRADES is still
                # TRADES.
                cst.Models.TRADES.value,
            )
            self.diffuser = MultiAssetGaussianDiffusion(
                config, asset_universe, self.feature_augmenter
            ).to(cst.DEVICE, non_blocking=True)
        else:
            self.diffuser = MultiAssetGaussianDiffusion(
                config, asset_universe, None
            ).to(cst.DEVICE, non_blocking=True)

        # Same fixed type embedding as the single-asset engine.
        self.type_embedder = nn.Embedding(3, self.size_type_emb, dtype=torch.float32)
        self.type_embedder.requires_grad_(False)
        self.type_embedder.weight.data = torch.tensor(
            [[0.4438, -0.2984, 0.2888],
             [0.8249, 0.5847, 0.1448],
             [1.5600, -1.2847, 1.0294]],
            device=cst.DEVICE, dtype=torch.float32,
        )
        if self.IS_WANDB:
            wandb.log({"type_embedder": self.type_embedder.weight.data}, step=0)

        self.ema = ExponentialMovingAverage(self.parameters(), decay=0.999)
        self.ema.to(cst.DEVICE)
        self.sampler = LossSecondMomentResampler(self.num_diffusionsteps)
        self.vlb_sampler = LossSecondMomentResampler(self.num_diffusionsteps)
        self.simple_sampler = LossSecondMomentResampler(self.num_diffusionsteps)
        self.save_hyperparameters(ignore=["asset_universe"])

    # ---- forward / sampling --------------------------------------------

    def forward(self, cond_orders, x_0, cond_lob, is_train, batch_idx=None):
        # x_0 shape: (B, N, K_gen, LEN_ORDER); cond_orders: (B, N, K_cond, LEN_ORDER)
        raw_cond_orders = cond_orders
        raw_cond_lob = cond_lob
        x_0, cond_orders = self.type_embedding(x_0, cond_orders)
        if is_train:
            self.t, _ = self.sampler.sample(x_0.shape[0], device=x_0.device)
            recon = self.single_step(
                cond_orders,
                x_0,
                cond_lob,
                batch_idx,
                raw_cond_orders=raw_cond_orders,
                raw_cond_lob=raw_cond_lob,
            )
        else:
            self.t = torch.full(size=(x_0.shape[0],),
                                fill_value=self.num_diffusionsteps - 1,
                                device=x_0.device, dtype=torch.int64)
            for _ in range(self.num_diffusionsteps - 1, -1, -1):
                recon = self.single_step(
                    cond_orders,
                    x_0,
                    cond_lob,
                    raw_cond_orders=raw_cond_orders,
                    raw_cond_lob=raw_cond_lob,
                )
                self.t = self.t - 1
        return recon

    def sample(self, **kwargs) -> torch.Tensor:
        raw_cond_orders: torch.Tensor = kwargs["cond_orders"]
        x_0: torch.Tensor = kwargs["x"]
        raw_cond_lob: torch.Tensor = kwargs["cond_lob"]
        cond_orders = raw_cond_orders
        cond_lob = raw_cond_lob
        x_0, cond_orders = self.type_embedding(x_0, cond_orders)
        x_0 = torch.zeros_like(x_0)
        weights = self.sampler.weights()
        return self.diffuser.sample(
            x_0,
            cond_orders,
            cond_lob,
            weights,
            graph_cond_orders=raw_cond_orders,
            graph_cond_lob=raw_cond_lob,
        )

    def single_step(
        self,
        cond_orders,
        x_0,
        cond_lob,
        batch_idx=None,
        raw_cond_orders=None,
        raw_cond_lob=None,
    ):
        x_t, noise = self.diffuser.forward_reparametrized(x_0, self.t)
        x_t_aug, cond_orders_aug, cond_lob_aug = self.diffuser.augment(x_t, cond_orders, cond_lob)
        weights = self.sampler.weights()
        return self.diffuser.ddpm_single_step(
            x_0,
            x_t_aug,
            x_t,
            self.t,
            cond_orders_aug,
            noise,
            weights,
            cond_lob_aug,
            batch_idx,
            raw_cond_orders=raw_cond_orders,
            raw_cond_lob=raw_cond_lob,
        )

    def type_embedding(self, x_0, cond):
        """Same as single-asset version but operating on the last (feature)
        dimension; works for either (B, K, F) or (B, N, K, F)."""
        order_type = x_0[..., 1]
        order_type_emb = self.type_embedder(order_type.long())
        x_0 = torch.cat((x_0[..., :1], order_type_emb, x_0[..., 2:]), dim=-1)
        cond_type = cond[..., 1]
        cond_depth_emb = self.type_embedder(cond_type.long())
        cond = torch.cat((cond[..., :1], cond_depth_emb, cond[..., 2:]), dim=-1)
        return x_0, cond

    # ---- loss ----------------------------------------------------------

    def loss(self):
        L_hybrid, L_simple, L_vlb = self.diffuser.loss()
        return L_hybrid, L_simple, L_vlb

    # ---- Lightning hooks (parity with single-asset DiffusionEngine) ----

    def training_step(self, input, batch_idx):
        if self.global_step == 0 and self.IS_WANDB:
            self._define_log_metrics()
        cond_orders = input[0].contiguous()
        x_0 = input[1].contiguous()
        cond_lob = input[2].contiguous()
        x_0.requires_grad_(True)
        cond_orders.requires_grad_(True)
        cond_lob.requires_grad_(True)
        if self.cond_type != "full":
            cond_lob = None
        _ = self.forward(cond_orders, x_0, cond_lob, is_train=True, batch_idx=batch_idx)
        batch_loss, L_simple, L_vlb = self.loss()
        self.simple_train_losses.append(torch.mean(L_simple).item())
        self.vlb_train_losses.append(torch.mean(L_vlb).item())
        batch_loss_mean = torch.mean(batch_loss)
        self.train_losses.append(batch_loss_mean.item())
        self.sampler.update_losses(self.t, batch_loss[0])
        self.vlb_sampler.update_losses(self.t, L_vlb[0])
        self.simple_sampler.update_losses(self.t, L_simple[0])
        self.diffuser.init_losses()
        self.ema.update()
        if batch_idx % 1000 == 0:
            print(f"batch loss: {batch_loss_mean}")
            print(f"graph gamma: {self.diffuser.graph_gamma.detach().item():.6f}")
        return batch_loss_mean

    def on_fit_start(self) -> None:
        # Under DDP the LightningModule is moved to each rank's device after
        # __init__; the EMA shadow params were captured on the construction
        # device, so realign them to the live module device here.
        self.ema.to(self.device)

    def on_train_epoch_start(self) -> None:
        print(f'learning rate: {self.optimizer.param_groups[0]["lr"]}')

    def on_validation_start(self) -> None:
        if not self.train_losses:
            return
        loss = sum(self.train_losses) / len(self.train_losses)
        L_simple = sum(self.simple_train_losses) / len(self.simple_train_losses)
        L_vlb = sum(self.vlb_train_losses) / len(self.vlb_train_losses)
        if self.IS_WANDB:
            wandb.log({
                "train loss simple": L_simple,
                "train loss vlb": L_vlb,
                "train_loss": loss,
            }, step=self.global_step)
            print(f"\ntrain loss simple on step {self.global_step} is {round(L_simple, 3)}")
            print(f"\ntrain loss vlb on step {self.global_step} is {round(L_vlb, 3)}")
            print(f"\ntrain loss on step {self.global_step} is {round(loss, 3)}")
        self.train_losses = []
        self.simple_train_losses = []
        self.vlb_train_losses = []
        self.val_ema_losses = []
        self.simple_val_losses = []
        self.vlb_val_losses = []

    def validation_step(self, input, batch_idx):
        cond_orders = input[0]
        x_0 = input[1]
        cond_lob = input[2]
        if self.cond_type != "full":
            cond_lob = None
        with self.ema.average_parameters():
            _ = self.forward(cond_orders, x_0, cond_lob, is_train=False)
            batch_loss, L_simple, L_vlb = self.loss()
            self.simple_val_losses.append(torch.mean(L_simple).item())
            self.vlb_val_losses.append(torch.mean(L_vlb).item())
            batch_loss_mean = torch.mean(batch_loss)
            self.val_ema_losses.append(batch_loss_mean.item())
        self.diffuser.init_losses()
        return batch_loss_mean

    def on_validation_epoch_end(self) -> None:
        if not self.val_ema_losses:
            return
        loss_ema = sum(self.val_ema_losses) / len(self.val_ema_losses)
        if loss_ema < self.min_loss_ema:
            if loss_ema - self.min_loss_ema > -0.002:
                self.optimizer.param_groups[0]["lr"] /= 2
            self.min_loss_ema = loss_ema
            self.model_checkpointing(loss_ema)
        else:
            self.optimizer.param_groups[0]["lr"] /= 2

        L_simple = sum(self.simple_val_losses) / len(self.simple_val_losses)
        L_vlb = sum(self.vlb_val_losses) / len(self.vlb_val_losses)
        if self.IS_WANDB:
            wandb.log({"val_loss_simple": L_simple}, step=self.current_epoch + 1)
            wandb.log({"val_loss_vlb": L_vlb}, step=self.current_epoch + 1)
        print(f"\nval loss simple on epoch {self.current_epoch} is {round(L_simple, 3)}")
        print(f"\nval loss vlb on epoch {self.current_epoch} is {round(L_vlb, 3)}")
        self.log("val_ema_loss", loss_ema)
        print(f"\n val ema loss on epoch {self.current_epoch} is {round(loss_ema, 3)}")

    def _graph_branch_params(self):
        """Parameters of the P1 graph-coupling branch on the diffuser.

        Returns (trunk_params, graph_params) where graph_params are the gate
        + edge/message/fusion/relation modules and trunk_params is everything
        else in the diffuser. The split lets the gate open on a much smaller
        LR so it cannot run away from its safe zero init before the MLPs are
        useful. Harmless under disable_graph (those params just get ~0 grad).
        """
        graph_modules = []
        for attr in ("relation_embedding", "edge_weight_net", "message_fn",
                     "aggregator", "noise_fusion"):
            mod = getattr(self.diffuser, attr, None)
            if mod is not None:
                graph_modules.append(mod)
        graph_params = [p for m in graph_modules for p in m.parameters()]
        graph_ids = {id(p) for p in graph_params}
        trunk_params = [p for p in self.diffuser.parameters() if id(p) not in graph_ids]
        return trunk_params, graph_params

    def configure_optimizers(self):
        trunk_params, graph_params = self._graph_branch_params()
        print(f"[optim] trunk_params={sum(p.numel() for p in trunk_params)} "
              f"graph_params={sum(p.numel() for p in graph_params)} "
              f"(graph_lr={self.graph_lr}, trunk_lr={self.lr})")
        # group 0 must stay the trunk: on_validation_epoch_end / lr logging
        # index param_groups[0].
        if self.optimizer == "Adam":
            self.optimizer = torch.optim.Adam(
                [
                    {"params": trunk_params},
                    {"params": graph_params, "lr": self.graph_lr},
                    {"params": self.type_embedder.parameters(), "lr": 0.01},
                ],
                lr=self.lr,
            )
        elif self.optimizer == "RMSprop":
            self.optimizer = torch.optim.RMSprop(
                [{"params": trunk_params},
                 {"params": graph_params, "lr": self.graph_lr}],
                lr=self.lr,
            )
        elif self.optimizer == "SGD":
            self.optimizer = torch.optim.SGD(
                [{"params": trunk_params},
                 {"params": graph_params, "lr": self.graph_lr}],
                lr=self.lr, momentum=0.9,
            )
        elif self.optimizer == "LION":
            self.optimizer = Lion(
                [{"params": trunk_params},
                 {"params": graph_params, "lr": self.graph_lr}],
                lr=self.lr,
            )
        return self.optimizer

    def _define_log_metrics(self):
        wandb.define_metric("val_loss", summary="min")
        wandb.define_metric("val_ema_loss", summary="min")

    def model_checkpointing(self, loss):
        if self.last_path_ckpt_ema is not None:
            os.remove(self.last_path_ckpt_ema)
        filename_ckpt_ema = (
            "val_ema=" + str(round(loss, 3))
            + "_epoch=" + str(self.current_epoch)
            + "_" + self.filename_ckpt
            + ".ckpt"
        )
        path_ckpt_ema = cst.DIR_SAVED_MODEL + "/" + str(self.chosen_model) + "/" + filename_ckpt_ema
        with self.ema.average_parameters():
            self.trainer.save_checkpoint(path_ckpt_ema)
        self.last_path_ckpt_ema = path_ckpt_ema
