...
class TRADES(nn.Module):
    def __init__(
        ...
        # Register the fixed sinusoidal embeddings as buffers so Lightning /
        # .to(device) relocates them per-process under DDP (plain attributes
        # stay on the construction device -> cuda:0 -> device mismatch).
        self.register_buffer("t_embedder", sinusoidal_positional_embedding(num_diffusionsteps, input_size))
        self.seq_size = gen_sequence_size + cond_seq_len
        self.register_buffer("pos_embed", sinusoidal_positional_embedding(self.seq_size, input_size))
