...
class MultiAssetDiffusionEngine(LightningModule):
    ...
    def forward(self, cond_orders, x_0, cond_lob, is_train, batch_idx=None):
        ...
        raw_cond_orders = cond_orders.detach()
        raw_cond_lob = cond_lob.detach() if cond_lob is not None else None
        x_0, cond_orders = self.type_embedding(x_0, cond_orders)
        if is_train:
            self.t, _ = self.sampler.sample(x_0.shape[0])
            recon = self.single_step(
                cond_orders, x_0, cond_lob, batch_idx,
    ...
    def sample(self, **kwargs) -> torch.Tensor:
        ...
        return self.diffuser.sample(
            x_0, cond_orders, cond_lob, weights,
            raw_cond_orders=raw_cond_orders, raw_cond_lob=raw_cond_lob,
        )
